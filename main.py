...
def generate_frames(frames, preserve_continuity=True):
    ...
    for i, frame_data in enumerate(frames):
        # Handle both old format (3-tuple) and new format (4-tuple with text)
        if len(frame_data) == 3:
            nodes, connections, duration = frame_data
            frame_text = None
        else:
            nodes, connections, duration, frame_text = frame_data
        ...
        clip = generate_frame(nodes, connections, duration, preserve_last=preserve_flag, frame_text=frame_text)
        clips.append(clip)
...
def generate_video_from_story(story_frames, duration_per_step=1.0, preserve_continuity=True):
    ...
    return generate_frames(all_frames, preserve_continuity=preserve_continuity)
