...
def get_raw_output(prompt):
    ...
    prompt_real = f"you will generate script for a educational video animations. " \
                   f"The video is supposed to properly explain and teach the topic: '{prompt}' for the video " \
                           "encoding, we have multiple options: so mostly you create videos with multiple " \
                           "frames each frame can either have graph containing some boxes and some connections for explaining" \
                           "the topic, or just some text. You define multiple frames to " \
                           "explain the topic for each frame, you give a frame_text, and the visualisation of that frame_text. " \
                           "In the visualisation, if the frame has graphs, you mention boxes with names and then draw connections. " \
                           ...
                           "the user understand the concept. The frame_text MUST mention the labels of the nodes. A sample connection text might look like node0,node1:node2;node2:node3;node3:node4 to mean" \
                           "node0 and node1 points to node2, node1 points to node2 and node2 points to node3. Use delimiter '$' between" \
                           ...
                           "YOU CANNOT USE SPECIAL CHARACTERS IN frame_text. DO NOT USE SPECIAL CHARACTER IN FRAME TEXT. Make frame_text very descriptive and make sure to use it to explain the graph in that frame. " \
                           "Make the whole content as if it is an end to end story, with frame_text as the story lines. Make atleast" \
                           "8 frames to explain the concept clearly. Make sure 20% of the frames don't have a graph, as in they have NO_NODE in place of node details. For example" \
